...
export const TradingViewChart: React.FC<TradingViewChartProps> = ({
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartContainerRef.current, {
      ...
      localization: {
        // Display time as-is (already adjusted to IST) without further conversion
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (sourceInfo.source === "binance") {
        // Crypto symbols use Binance API (real-time WebSocket)
        url = `/api/binance/candles?symbol=${encodeURIComponent(symbol)}&interval=${tf.interval}&range=${tf.range}`;
      } else {
        // Everything else uses Yahoo API (stocks, indices, forex, commodities)
      ...
      for (const p of json.points) {
        const time = (p.t + IST_OFFSET) as UTCTimestamp;
    ...
    const connectWebSocket = () => {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          // IST offset: UTC+5:30 = 5.5 hours = 19800 seconds
          const IST_OFFSET = 5.5 * 60 * 60;
          const time = (Math.floor(k.t / 1000) + IST_OFFSET) as UTCTimestamp;
