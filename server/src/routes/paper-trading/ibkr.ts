...
router.post("/order", async (req: Request, res: Response) => {
  try {
    ...
    const searchResult = await ibkrRequest(
      `/iserver/secdef/search?symbol=${encodeURIComponent(symbol)}&name=true`
    );
    ...
    // Place order
    const orderResult = await ibkrRequest(
    ...
    if (orderResult[0]?.id && orderResult[0]?.message) {
      ...
      const confirmResult = await ibkrRequest(
        `/iserver/reply/${orderResult[0].id}`,
        "POST",
        { confirmed: true }
      );
