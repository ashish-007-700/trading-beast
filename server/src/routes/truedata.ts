...
type TrueDataBar = {
  ...
  c: number;     // close
  v: number;     // volume
  oi?: number;   // open interest (optional, for F&O)
};

type TrueDataResponse = {
  success: boolean;
  status: string;
  data?: TrueDataBar[];
  error?: string;
...
function mapIntervalToBarSize(interval: string): string {
...
function mapRangeToDuration(range: string): string {
  const map: Record<string, string> = {
    '1d': '1 D',
    '5d': '5 D',
    '1mo': '1 M',
...
router.get('/candles', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const data = await response.json() as TrueDataResponse;
