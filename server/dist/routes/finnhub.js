...
router.get('/candles', async (req, res) => {
    ...
    const interval = req.query.interval?.trim();
    const range = req.query.range?.trim();
    const resolution = interval ? mapIntervalToResolution(interval) : (req.query.resolution?.trim() || 'D');
    const now = new Date();
...
router.get('/news', async (req, res) => {
    ...
    const fromDate = req.query.from || weekAgo.toISOString().split('T')[0];
